...
for anime, id in data.items():
    try:
        with open(filter(anime) + '/index.html') as file:
            page = file.read()
   ...
   <div class="close-this-a large-txt" onclick="hide_menu()">
    ...
    <div class="close-me-div" onclick="hide_menu()"></div>"""
        ...
        with open(filter(anime) + '/index.html', 'w') as file:
            file.write(soup.prettify())

        with open(filter(anime) + '/Watch-Now/index.html') as file:
            page = file.read()
            soup = BeautifulSoup(page, 'html.parser')
            soup.find('html')['id'] = id
   ...
   <div class="close-this-a large-txt" onclick="hide_menu()">
    ...
    <div class="close-me-div" onclick="hide_menu()"></div>"""
            ...
            if script_to_remove:
                script_to_remove.decompose()
        with open(filter(anime) + '/Watch-Now/index.html', 'w') as file:
            file.write(soup.prettify())
