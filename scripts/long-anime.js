...
// let animeNew = anime.replace(" ","");
        ...
        for(let i = min_range; i <= max_range; i++){
            ...
            if(i == epi_num){
                temp = `<div class='episode active ${fillerFlag}' title="${episodeNameNew}" id='` + anime + "-episode-" + i + "'>" + i + "</div>";
                ...
                temp = `<div class='episode ${fillerFlag}' title="${episodeNameNew}" id='` + anime + "-episode-" + i + "' onclick='" + temp_1 + "'>" + i +"</div>" ;
    ...
    if(newAnimesName.hasOwnProperty(animeNameee.replaceAll(" ", ""))){
        Object.keys(short_anime_data).forEach( lang => {
            ...
            for(let i = 1 ; i <= current_short_anime_epi_num ; i ++){
                ...
                if(i == epi_num){
                    temp = ` <div class="episode-short ${fillerFlag}" title="${episodeNameNew}"><div class="short-anime-ep active-episode" id="` + anime + '-episode-' + i + '"> ' + 
                    ...
                    temp = `<div class="episode-short ${fillerFlag}" title="${episodeNameNew}" onclick="` + temp_1 + '"><div class="short-anime-ep"  id="' + anime + '-episode-' + i + '"><div class="short-anime-num episode-alternate-' + first + '">' + i +'</div><div class="short-anime-name episode-alternate-' + second + '"><span>' + episodeNameNew + '</span></div></div></div>'
                }
                outputNew += temp;
            }
            episodes_section.innerHTML = outputNew;
